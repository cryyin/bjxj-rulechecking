...
def read_calculation(path):
    ...
    flags = []
    for i,p in enumerate(file.paragraphs):
        if p.text.endswith("设计依据") or p.text.endswith("计算依据") or p.text.endswith("设计遵照的规范"):
            flags.append(i)
    ...
    if len(flags)>1:
        ...
        for flag in flags:
            ...
            if starts!=-1 and ends!=-1:
                ...
                #data["6.钢支撑计算"][target_tt] = []
                #for i,cont in enumerate(range(starts,ends)):
                #    data["6.钢支撑计算"][target_tt].append(file.paragraphs[cont].text) 
                #print(target_tt,data["6.钢支撑计算"][target_tt],"\n\n")
                flagt1=-1
                flagt2=-1
                flagt3=-1
                flagt4=-1
                ...
                for i,cont in enumerate(range(starts,ends)):
                    if "钢支撑刚度验算" in file.paragraphs[cont].text:
                        flagt1 = starts+i
                    if "钢支撑强度验算" in file.paragraphs[cont].text:
                        flagt2 = starts+i
                    if "钢支撑整体稳定性验算" in file.paragraphs[cont].text:
                        flagt3 = starts+i
                    if "钢支撑挠度验算" in file.paragraphs[cont].text:
                        flagt4 = starts+i

                if flagt1!=-1 and flagt2!=-1 and flagt3!=-1 and flagt4!=-1:
                    data["钢支撑计算"][target_tt]["刚度"] = ""
                    for i,cont in enumerate(range(flagt1+1,flagt2)):
                        data["钢支撑计算"][target_tt]["刚度"] += file.paragraphs[cont].text
                    data["钢支撑计算"][target_tt]["强度"] = ""
                    for i,cont in enumerate(range(flagt2+1,flagt3)):
                        data["钢支撑计算"][target_tt]["强度"] += file.paragraphs[cont].text
                    data["钢支撑计算"][target_tt]["稳定性"] = ""
                    for i,cont in enumerate(range(flagt3+1,flagt4)):
                        data["钢支撑计算"][target_tt]["稳定性"] += file.paragraphs[cont].text
                    data["钢支撑计算"][target_tt]["挠度"] = ""
                    for i,cont in enumerate(range(flagt4+1,ends)):
                        data["钢支撑计算"][target_tt]["挠度"] += file.paragraphs[cont].text
...
def read_CAD_results(path):
    ...
    for dir in os.listdir(path):
        ...
        for file in os.listdir(os.path.join(path, dir)):
            with open(os.path.join(path, dir, file), 'r', encoding='utf8') as f:
                content=json.load(f)
                dir_dict[file.split('.')[0]]=content
        contents[dir]=dir_dict
